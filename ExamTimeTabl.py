...
# Inputs: exam start and end dates
start_date_str = '2025-09-26'
end_date_str = '2025-10-06'
...
while current_date <= end_date:
    if current_date.weekday() != 6 and current_date not in public_holidays:  # weekday 6 = Sunday
...
# -----------------------------
sheet_year_map = {}
year_names = ["1St Year", "2Nd Year", "3Rd Year", "4Th Year", "5Th Year"]  # extend if needed
...
for sheet_name, df in course_book.items():
    year = sheet_year_map[sheet_name]  # 1St Year, 2Nd Year, etc.
    branch_courses[year] = {}
    # Standardize branch columns
...
for _, row in df_common.iterrows():
    # Normalize year
    year_norm = normalize_year(str(row['Year']))  # "1" -> "1St Year"

    # Make sure 'Branches' is treated as string, then split by comma
...
# Track which common courses have been assigned
common_assigned = {code: False for code in common_course_map}

# -----------------------------
# Step 6b: Assign Common Courses (blocking respective branches)
# -----------------------------
for day in days:
    for course_code, info in common_course_map.items():
        year_norm = info['Year']
        branches_to_block = info['Branches']  # This MUST be branch codes
        total_students = sum(branch_strength[year_norm][b] for b in branches_for_course)
        ...
        for slot in slots:
            ...
            for branch in branches_to_block:
                if day_year_credits[day][year_norm].get(branch, 0) + info['credits'] > 5:
                    conflict = True
                    break
            ...
            break
# -----------------------------
# Step 7: Assign Main Courses (Default Branch Allocation & Capacity Check)
# -----------------------------
...
for year in default_branch_slot_allocation:
    ...
    for slot in default_branch_slot_allocation[year]:
        ...
        if normalized_year not in branch_strength:
            print(f"⚠️ Warning: {normalized_year} not found in branch_strength keys {list(branch_strength.keys())}")
            continue
...
# Apply same allocation for all days (deep copy)
...
for day in days:
    for slot in slots:
        for year in years:
            ...
            # Track how many students already in this slot
            ...
            # Update the allocation to only include branches that fit
            branch_slot_allocation_day[day][normalized_year][slot] = final_branches


print("\n Using Branch-Slot Allocation for", len(days), "days")
print(branch_slot_allocation_day[days[0]])  # show one example day
...
for day in days:
    ...
    for slot in slots:
        # 1️⃣ Determine blocked branches for this slot/day
        blocked_branches = set()
        for year in years:
            for c in schedule[day][slot][year]:
                if c.get("type") == "Common":
                    # Add each branch in the common course
        ...
        for year in years:
            ...
            for b in candidates:
                ...
                day_slot_total_courses[day][slot] += 1
        # -----------------------------
        # Fill empty slots after ENV day if courses remain
        # -----------------------------
...
for sheet_name, df in student_book.items():
    # Normalize year: remove " Year" and standardize
    year_key = normalize_year(sheet_name.replace(" Year", ""))  # MUST match schedule keys
    for branch in ["CSE", "DSAI", "ECE"]:
        ids = df[branch].dropna().astype(str).tolist()
...
for day in days:
    ...
    for slot in slots:
        ...
        for course in courses_in_slot:
            ...
            year_label = normalize_year(course["year"])  # ✅ important
            ...
            for sr in student_ranges:
                ...
                for r in room_names:
                    if len(room_courses[r]) < 2 and course["course_code"] not in room_courses[r]:
                        ...
                        break
                if not target_room:
                    print(f"No available room for {course['course_code']} ({sr}) on {day} {slot}")
                    continue
...
print("Main exam timetable formatted with colors and auto-adjusted sizes successfully!")
# -----------------------------
# Step 13: Create Free Slot Sheet with Room Availability and Remaining Capacity
# -----------------------------
...
for day in days:
    for slot in slots:
        # Get rooms already occupied in this slot
        ...
        # Remaining capacity in this slot
        ...
        for year in years:
            for branch in branches:
                ...
                free_rows.append({
                    ...
                    # Only show available rooms and capacity once per slot
...
print("Free slot sheet with available rooms and remaining capacity saved successfully!")
wb = load_workbook("exam_schedule_with_rooms_faculty.xlsx")
...
# List all sheets you want to format
...
for sheet_name in sheet_names:
    ...
    # Auto-adjust row heights and center alignment
    for row_cells in ws.iter_rows(min_row=2, max_row=ws.max_row):
        for cell in row_cells:
            cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
            ws.row_dimensions[cell.row].height = 30

    # Apply branch colors for any cell containing branch code
    for row_cells in ws.iter_rows(min_row=2, max_row=ws.max_row):
        for cell in row_cells:
...
# Center alignment and row height
for row in ws_main.iter_rows(min_row=2, max_row=ws_main.max_row):
    for cell in row:
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
        ws_main.row_dimensions[cell.row].height = 30
...
branch_colors = {
    "CSE": "FFC7CE",   # light red
    "DSAI": "C6EFCE",  # light green
    "ECE": "FFEB9C",   # light yellow
    "All": "BDD7EE"    # light blue for common course
}

# Apply color based on branch in the cell value
for row in ws_main.iter_rows(min_row=2, max_row=ws_main.max_row):
    for cell in row:
        for branch, color in branch_colors.items():
            if branch in str(cell.value):
...
# Save the workbook
wb.save("exam_schedule_with_rooms_faculty.xlsx")
print("Exam Schedule formatted with colors successfully!")